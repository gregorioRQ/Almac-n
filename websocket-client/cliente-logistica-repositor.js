...
function createCard(message) {
  ...
  // Crear formulario para editar la propiedad "ubicacion"
  const form = document.createElement("form");
  form.classList.add("card-item");

  const label = document.createElement("label");
  label.innerText = "Ubicacion del ítem:";
  label.setAttribute("for", "ubicacionInput");
  form.appendChild(label);

  const input = document.createElement("input");
  input.type = "text";
  input.id = "ubicacionInput";
  input.value = ""; // Valor actual de "estado"
  ...
  button.addEventListener("click", () => {
    // Actualizar el valor de "ubicacion" en el JSON recibido
    message.ubicacion = input.value;
...
// decide si renderizar una nueva tajeta o actualizarla.
...
function connect() {
  ...
  stompClient.connect(
    ...
    function (frame) {
      ...
      //suscripcion para mensajes de advertencia
      stompClient.subscribe("/topic/repositor", onRepositorMessageReceived);

      // Enviar un mensaje vacio al backend para solicitar el historial
...
// renderiza la notificaciones
function onRepositorMessageReceived(payload) {
  const notification = JSON.parse(payload.body);
  console.log("Mensaje de Repositor Recibido:", notification);
...
function sendMessage(updatedMessage) {
  ...
  stompClient.send(
    "/app/chat.guardar", // Endpoint del servidor para manejar el mensaje
    {},
    JSON.stringify(updatedMessage) // Convertir el objeto a JSON
  );
  console.log("Datos enviados al servidor:", updatedMessage);
}
